...
Tension between two ASTs (one with "invalid" syntax and one fully valid) and one more complex AST.
- similar tradeoffs as with compiler ASTs/compilation passes
- type safety, maintenance & code complexity tradeoffs
- at the moment: a single AST

**Security & Sanitization.**

(TODO more details)

Duplicated headers are dropped. Keeping duplicated headers (forbidden by the RFC) would be a security issue (ref?)

**Missing world knowledge & use cases.**

We have seen when testing on real datasets that some errors are very common. So, even if they are invalid, we should assume they are part of the spec. And because we don't have access to billion of emails, we may miss some of these patterns. 

Also, some features or properties are way more important than others, based on use cases / existing clients & mail software, and thus requires a good knowledge of existing software processing email representations (eg. some emails clients may be very sensitive to a specific field).

Recovery strategies are biased in favor of the corpus we have tested. Indeed, recovery is de-facto outside of the spec. Currently, we are aware that we risk overfitting our recovery strategies to pass on our known dataset. We are always open to extending eml-codec to work on more real-world emails if presented with new datasets (these are not so easy to come by as most emails are private data).

## Arbitration and Compromises

There is no "one size fits all" solution that always satisfies all of our design principles. So we keep a list of all the arbitration we did in the implementation and why.

#### Duplicated header behavior

We keep the 1st header.
We drop the other ones.

Note: Thunderbird keeps the first header.

Recommendation: use a single parser for your whole stack to make sure you have always the same behavior.
(eg. HTTP reverse proxy that have a security role and trusted backend that do not interpret HTTP the same way)

#### `message-id` are often wrong

We chose to relax the syntax that we parse instead of discarding.
Too many message-id are invalid in practice (eg. Apple Mail).

@TODO example (message id Apple Mail)

**We favor [C] over [B].**

#### IMAP Draft allows missing fields (eg. `Date` & `From`) 

First we replaced these missing fields (mandatory according to the RFC) by dummy values.

But IMAP email drafts allow them to be missing.
So we relaxed that.

This is extended to unreadable From and Date fields (they are dropped and treated as missing).
